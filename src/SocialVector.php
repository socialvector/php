...
use SocialVector\Logger\Logger;
use SocialVector\HttpClient\HttpClientInterface;

/**
 * Social Vector PHP
 *
 * For ease of use of multiple providers, SocialVector implements the class SocialVector\SocialVector,
 * a sort of factory/façade which acts as a unified interface or entry point, and it expects a
...
class SocialVector
{
    ...
    public function __construct(
        ...
        if (is_string($config) && file_exists($config)) {
            ...
            throw new InvalidArgumentException('Social Vector config does not exist on the given path.');
        }
     ...
     *
     * @return \SocialVector\Adapter\AdapterInterface
     * @throws InvalidArgumentException
    ...
    public function getAdapter($name)
    {
        ...
        $adapter = isset($config['adapter']) ? $config['adapter'] : sprintf('SocialVector\\Provider\\%s', $name);
            ...
            foreach ($fs as $file) {
                if (!$file->isDir()) {
                    ...
                    if ($name === mb_strtolower($provider)) {
                        $adapter = sprintf('SocialVector\\Provider\\%s', $provider);
                        break;
                    }
                }
            }
